...
class IntegrationStrategy : Strategy {
    ...
    override fun execute(input: String): String {
        val appid = "" // Replace wuth your appid
        var response =""
        var actual_response ="" // DELETE LATER

        var adjusted_input = input.replace("+", "%2B")
        adjusted_input = adjusted_input.replace(" ", "+")
        val url = URL("http://api.wolframalpha.com/v2/query?input=integrate(${adjusted_input})&appid=$appid")  //TODO Whitespace replacement
            ...
            inputStream.bufferedReader().use {
                it.lines().forEach { line ->
                    run {
                        response += line
                        println(line)
                        if(line.contains("integral") && line.contains("plaintext")) actual_response = line // КОСТЫЛЬ
                    }
                }
